...
func (cs *chainSet) Initialize(testName string, homeDir string, client *client.Client, networkID string) error {
	var eg errgroup.Group

	for c := range cs.chains {
		c := c
		eg.Go(func() error {
			if err := c.Initialize(testName, homeDir, client, networkID); err != nil {
