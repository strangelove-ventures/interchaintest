...
func TestOsmosisExpeditedProposal(t *testing.T) {
	...
	nonExpeditedProposalTx, err := chain.TextProposal(ctx, chainUser.KeyName, ibc.TextProposal{
	...
	expeditedProposalTx, err := chain.TextProposal(ctx, chainUser.KeyName, ibc.TextProposal{
	...
	err = chain.VoteOnProposalAllValidators(ctx, nonExpeditedProposalTx.ProposalID, ibc.ProposalVoteYes)
	require.NoError(t, err, "failed to submit votes")

	err = chain.VoteOnProposalAllValidators(ctx, expeditedProposalTx.ProposalID, ibc.ProposalVoteYes)
	require.NoError(t, err, "failed to submit votes")

	err = test.PollForProposalStatus(ctx, chain, postProposalHeight, postProposalHeight+10, expeditedProposalTx.ProposalID, ibc.ProposalStatusPassed)
	...
	require.Equal(t, ibc.ProposalStatusVotingPeriod, nonExpeditedProposal.Status, "non-expedited proposal is not in voting period")
}
