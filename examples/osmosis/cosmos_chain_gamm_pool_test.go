...
func TestOsmosisGammPool(t *testing.T) {
	...
	poolID, err := cosmos.OsmosisCreatePool(chain, ctx, chainUser.KeyName, cosmos.OsmosisPoolParams{
	...
	_, err = cosmos.OsmosisSwapExactAmountIn(chain, ctx, chainUser.KeyName, fmt.Sprintf("50000%s", ibcDenom), "45000", []string{poolID}, []string{chainDenom})
