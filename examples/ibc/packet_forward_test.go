...
func TestPacketForwardMiddleware(t *testing.T) {
	...
	t.Run("multi-hop a->b->c->d", func(t *testing.T) {
		...
		transfer := ibc.WalletAmount{
			Address: userB.GetFormattedAddress(chainB.Config().Bech32Prefix),
		...
		secondHopMetadata := &PacketMetadata{
			Forward: &ForwardMetadata{
				Receiver: userD.GetFormattedAddress(chainD.Config().Bech32Prefix),
		...
		firstHopMetadata := &PacketMetadata{
			Forward: &ForwardMetadata{
				Receiver: userC.GetFormattedAddress(chainC.Config().Bech32Prefix),
		...
		transferTx, err := chainA.SendIBCTransfer(ctx, abChan.ChannelID, userA.GetKeyName(), transfer, ibc.TransferOptions{Memo: string(memo)})
		require.NoError(t, err)
		...
		chainABalance, err := chainA.GetBalance(ctx, userA.GetFormattedAddress(chainA.Config().Bech32Prefix), chainA.Config().Denom)
		require.NoError(t, err)

		chainBBalance, err := chainB.GetBalance(ctx, userB.GetFormattedAddress(chainB.Config().Bech32Prefix), firstHopIBCDenom)
		require.NoError(t, err)

		chainCBalance, err := chainC.GetBalance(ctx, userC.GetFormattedAddress(chainC.Config().Bech32Prefix), secondHopIBCDenom)
		require.NoError(t, err)

		chainDBalance, err := chainD.GetBalance(ctx, userD.GetFormattedAddress(chainD.Config().Bech32Prefix), thirdHopIBCDenom)
		require.NoError(t, err)
	...
	t.Run("multi-hop denom unwind d->c->b->a", func(t *testing.T) {
		...
		transfer := ibc.WalletAmount{
			Address: userC.GetFormattedAddress(chainC.Config().Bech32Prefix),
		...
		secondHopMetadata := &PacketMetadata{
			Forward: &ForwardMetadata{
				Receiver: userA.GetFormattedAddress(chainA.Config().Bech32Prefix),
		...
		firstHopMetadata := &PacketMetadata{
			Forward: &ForwardMetadata{
				Receiver: userB.GetFormattedAddress(chainB.Config().Bech32Prefix),
		...
		transferTx, err := chainD.SendIBCTransfer(ctx, dcChan.ChannelID, userD.GetKeyName(), transfer, ibc.TransferOptions{Memo: string(memo)})
		require.NoError(t, err)
	...
	t.Run("multi-hop through native chain ack error refund", func(t *testing.T) {
		...
		transfer := ibc.WalletAmount{
			Address: userA.GetFormattedAddress(chainA.Config().Bech32Prefix),
		...
		transferTx, err := chainB.SendIBCTransfer(ctx, baChan.ChannelID, userB.GetKeyName(), transfer, ibc.TransferOptions{})
		require.NoError(t, err)
		...
		chainABalance, err := chainA.GetBalance(ctx, userA.GetFormattedAddress(chainA.Config().Bech32Prefix), baIBCDenom)
		require.NoError(t, err)
		...
		// assert balances for user controlled wallets
		chainDBalance, err := chainD.GetBalance(ctx, userD.GetFormattedAddress(chainD.Config().Bech32Prefix), cdIBCDenom)
		require.NoError(t, err)

		chainCBalance, err := chainC.GetBalance(ctx, userC.GetFormattedAddress(chainC.Config().Bech32Prefix), bcIBCDenom)
		require.NoError(t, err)

		chainBBalance, err := chainB.GetBalance(ctx, userB.GetFormattedAddress(chainB.Config().Bech32Prefix), chainB.Config().Denom)
		require.NoError(t, err)
