...
func sendIBCTransfersFromBothChainsWithTimeout(
	...
	eg.Go(func() (err error) {
		for i, channel := range channels {
			...
			if err := chainutil.WaitForBlocks(ctx, 1, srcChain); err != nil {
				return err
			}
		}
		return nil
	...
	eg.Go(func() (err error) {
		for i, channel := range channels {
			...
			if err := chainutil.WaitForBlocks(ctx, 1, dstChain); err != nil {
				return err
			}
		}
		return nil
...
func preRelayerStart_HeightTimeout(ctx context.Context, t *testing.T, testCase *RelayerTestCase, srcChain ibc.Chain, dstChain ibc.Chain, channels []ibc.ChannelOutput) {
	...
	require.NoError(t, chainutil.WaitForBlocks(ctx, 15, srcChain, dstChain), "failed to wait for blocks")
}
...
func testPacketRelaySuccess(
	...
	for i, srcTx := range testCase.TxCache.Src {
		...
		srcAck, err := chainutil.PollForAck(ctx, srcChain, srcTx.Height, srcTx.Height+pollHeightMax, srcTx.Packet)
	...
	for i, dstTx := range testCase.TxCache.Dst {
		...
		dstAck, err := chainutil.PollForAck(ctx, dstChain, dstTx.Height, dstTx.Height+pollHeightMax, dstTx.Packet)
...
func testPacketRelayFail(
	...
	for i, srcTx := range testCase.TxCache.Src {
		...
		timeout, err := chainutil.PollForTimeout(ctx, srcChain, srcTx.Height, srcTx.Height+pollHeightMax, srcTx.Packet)
		...
		require.NoError(t, chainutil.WaitForBlocks(ctx, 2, srcChain, dstChain))
	...
	for i, dstTx := range testCase.TxCache.Dst {
		...
		timeout, err := chainutil.PollForTimeout(ctx, dstChain, dstTx.Height, dstTx.Height+pollHeightMax, dstTx.Packet)
