...
func SendFundsTx(api *gsrpc.SubstrateAPI, senderKeypair signature.KeyringPair, amount ibc.WalletAmount) (gstypes.Hash, error) {
	...
	call, err := gstypes.NewCall(meta, "Balances.transfer", receiver, gstypes.NewUCompactFromUInt(amount.Amount.Uint64()))
	if err != nil {
		return hash, err
	}
...
func SendIbcFundsTx(
	...
	amount2 := gstypes.NewU128(*big.NewInt(amount.Amount.Int64()))
...
func MintFundsTx(
	...
	assetId := gstypes.NewU128(*big.NewInt(assetNum))
	amount2 := gstypes.NewUCompactFromUInt(amount.Amount.Uint64())
