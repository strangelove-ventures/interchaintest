...
func (s *SidecarProcess) Name() string {
	if s.validatorProcess {
		return fmt.Sprintf("ict-%s-%s-val-%d-%s", s.Chain.Config().ChainID, s.ProcessName, s.Index, dockerutil.SanitizeContainerName(s.TestName))
	}

	return fmt.Sprintf("ict-%s-%s-%d-%s", s.Chain.Config().ChainID, s.ProcessName, s.Index, dockerutil.SanitizeContainerName(s.TestName))
}
