...
func (tn *ChainNode) QueryProposal(ctx context.Context, proposalID string) (string, error) {
	...
	status, ok := proposal["status"]
	if !ok {
		return "", fmt.Errorf("status does not exist on proposal: %+v", proposal)
	}
