...
WHERE type = "/ibc.core.client.v1.MsgCreateClient" AND chain_id = ?;`
	...
	t.Run("create connections", func(t *testing.T) {
		...
		require.NoError(t, chainutil.WaitForBlocks(ctx, 2, gaia0, gaia1))
		...
		require.NoError(t, chainutil.WaitForBlocks(ctx, 1, gaia0, gaia1))
...
`
	...
	t.Run("relay acknowledgement", func(t *testing.T) {
		...
		require.NoError(t, chainutil.WaitForBlocks(ctx, 2, gaia1))
